...
|-----|-----|-----|-----|
| imageVariant | Python version: | string | 3.14-trixie |


This template sets up a Python development environment with a local SQL Server 2025 container and a sample database. The database schema lives in a SQL Database project that targets Azure SQL Database.
...
- `app` is the container you work in. It uses `mcr.microsoft.com/devcontainers/python` on Debian 13 (trixie).
...
1. Select **Show All Definitions...**, type **Azure SQL**, and select the `python` template.
...
|---|---|
| `3.14-trixie` (default) | Python 3.14 on Debian 13. |
| `3.13-trixie` | Python 3.13 on Debian 13. |
...
The template installs `ms-mssql.mssql`. Its extension pack adds the SQL Database Projects extension. The template also installs the Python extensions. See `.devcontainer/devcontainer.json` for the full list.
...
The container has `mssql-python`, Microsoft's Python driver for SQL Server and Azure SQL Database. It brings its own driver layer, so you don't install an ODBC driver on the system.
...
`forwardPorts` in `.devcontainer/devcontainer.json` forwards ports `5000` and `1433`. Add the ports your app uses there. Use `forwardPorts` instead of `ports` in `docker-compose.yml`, because only `forwardPorts` works in Codespaces.
